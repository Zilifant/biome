...
⚠⚠ **No single line is attributable, and both attempts say why.** A 5-seed
decomposition at 6000 ticks reads off 40.8 / `climbs` 39.8 / `cue` 41.6 / both 46.8;
re-run at the **full 15 000-tick horizon** (3 seeds) it reads off 322.3 / `climbs`
289.3 / `cue` 310.3 / both 385.7 — per-seed ordering arbitrary, and **seed 1 reverses
the sign** (the shipped arm's 227 against its own control's 246). The within-arm
spread (216–505 for the control alone) dwarfs every between-arm difference: the
population effect is **seed-dominated**.

What survives is the **direction**, from two independent samples (+20% on 3 seeds,
+41% on 10), which matches the pool-growth story. What does not survive is treating
+41% as a number to tune against. ⚠ The next step for anyone who wants to is
**per-seed pairs** — `sweep --json` carries `seedRecords`, so "how many of the ten
seeds moved up" can be counted instead of inferred from a mean. This is **D14** at a
larger scale: a ten-seed mean is a signal and a three-seed bisection of it is noise.

⚠ **No line was dropped, and that was a real choice.** Removing the habitat cue was
the live option — its only claim is the one **D40** retired — but doing so on the
strength of an unattributable difference is precisely the tuning-on-noise the numbers
above argue against, and the cue is the only expression of `vulture.md`'s roosting
request this engine can carry. **Until those pairs are counted, V1 makes
flight-without-its-counterweight worse rather than better** — the combination the
retired plan named as the one to avoid — so the slow life history (**A78**) stops
being optional.
...
`carcass.decayTicks` and the vulture's discovery network (**A77**) both would**.
...
| A83 | **The ethologist was cited as evidence about mechanisms it could not see** _(2026-08-06)_ | ⚠⚠ **The generalisation is the item.** `npm run ethologist` was cited **twice** as evidence the behaviour plan's mechanisms behaved — "reports no new anomaly kind across six worlds" — when it read no position for finiteness, read `stamina` nowhere at all, and structurally excluded `defend`/`chase` from every detector (their targets move evasively, so "made no progress" is their normal case). **A clean report from a detector that cannot see the mechanism is not evidence about the mechanism** — D40's null arm and D31's tautological fixture in a third costume. The standing rule: a phase that adds a mechanism must extend this tool or record that it did not. ✅ **Detector family 4 shipped the same day** — `non-finite-state` (now including `social.pullScale`, which is a *divisor*, so 0 is as fatal as NaN and does not look it), `commitment-past-ceiling`, `sprint-to-exhaustion`, and `lost-from-its-band` — 28 tests, **17 mutations each failing the suite**. ⛔ **Two of the plan's four risks were measured and deliberately not built**, because neither can discriminate: "locked on one bearing" reaches **0.999** net/path in a *healthy* world at 600 ticks, and at 1500 the null arm separates the mechanism (on max 0.567 / p99 0.506, all wildebeest; off 0.522 / 0.457, mixed) with only **0.017 of headroom** above the control; "a band collapsed to a point" is prevented by `locomotion.maxOccupantsPerCell`, whose geometric floor healthy records already sit on (8 members → 1.39 minimum, 16 → 2.01). ⬜ What is still open is the tool's **live-world calibration**: the unit suite proves each detector *can* fire, only a sweep shows whether it fires too often. See `ACTION-ITEMS.md` A83 |
...
| A33 | **Mobbing** — prey collectively attacking a predator                                                                        | **Closed 2026-07-30** (built phase 10, demonstrated phase 11). ⚠ Not a new action: it is the *groupmate* half of `defend`, which §7 Decision had described since Step 23 with only the kin half implemented. The buffalo declares `behavior.mobWeight` and a mobbed hunt drops the lion's mean capture chance 0.508 → 0.277 (§9 Hunting). ⚠ Phase 11 corrected one thing phase 10 got wrong: the hunted animal **stands its ground** too, because a fleeing target is carried away from the herd by the chase and no mob ever reaches the attempt. ⚠⚠ **Extended 2026-08-06 (BEHAVIOR-PLAN P9), and the rarity is unchanged**: a defender now *charges* and then *pursues*, but a demo buffalo still chooses `defend` about **25 ticks in 4000**, so anything measured about mobbing on this roster is a small-sample reading — the P9 block asserts the mechanism in a sandbox and `test/cooperation.test.js`'s batch-2 seed list had to widen to six for its thinnest cell to reach n=10 |
...
⚠ **A tree is a terrain code, and the three alternatives were rejected for
reasons that will recur** for any future layer:

| option | verdict |
| --- | --- |
| **terrain code** (chosen) | Every chokepoint already reads terrain — `speedModifierAt`, `concealmentAt`, `isShelteredAt`, `SHELTERING_BY_CODE` in the hot scan — and ⚠ `habitat` weights are keyed by **the legend's own names**, so `habitat: { tree: 1.5 }` works with **zero engine code** the moment the legend has the entry. A tree is one `Uint8Array` value |
| a sparse `TreeGrid` beside `FeatureGrid` | Architecturally tidy and unaffordable: the shelter cue and the habitat gradient would each need a second grid read per cell of the hottest loop in the engine. That is the +56%-of-a-tick measurement in §9 Perception, which is now a standing rule — **nothing in the cell scan may consult a second grid** |
| `plant`-kind entities (**A3**) | Nothing needs per-tree identity, and every consumer would trade an array index for a spatial query |
| the dynamic shrub layer (**A51**) | Correct eventually, and three phases of work before a leopard can sit in one. Trees are the thicket-shaped **static MVP** of the woody layer, exactly as `THICKET` is |

**The cost of the choice, stated rather than discovered later:** terrain is static
and unsaved, regenerated from the seed on load — so a tree can never grow, burn
down, or be eaten. That is A51's boundary.
...
the gazelle gap halved and the leopard's reversed to −0.4.

⚠⚠ **One structural contributor is identified, and it is a general property rather
than a fact about trees: adding a terrain code *removes* preferred habitat.**
Every herbivore weights `ground` at 1.1–1.2 and none names `tree`, so converting
open ground to trees quietly shrinks the preferred habitat of every grazer in the
world — and **any** future terrain type does the same to every species that
enumerates terrain by name, because an unnamed terrain resolves to neutral 1 while
the ground it replaced was above 1. The fix is one weight per species per new code.
⚠ **Still open for the four herbivores** (**A79**): the leopard named `tree` at T3
and the vulture at V1, so two of eight species have caught up and the artifact is
live for the rest. It was deliberately not fixed at T1, where it would have moved
four species' habitat inside the gate measuring one terrain type.
...
⚠⚠ **There was a fourth thing it should have moved and did not, and that was the
whole change** _(fixed 2026-08-06, A84 / D49)_. The two refusals above are about
what the radius must not be allowed to reach. `behavior.herdDistance` is the
opposite case: it is what the radius **had to** reach and didn't. The radius
decides who is in the herd; the distance decides how close to the herd's centre
each of them insists on standing, and the pair is a **density**. Widening one and
leaving the other at the 2.0 chosen for a six-cell herd asked a hundred wildebeest
to fit inside a disc that holds twenty-five at `locomotion.maxOccupantsPerCell: 2`
— so 37.5% of their steps were refused, 9.3% of their ticks had no legal step in
any direction at all, and they starved standing on forage while the population fell
57% (the buffalo, whose intake is mass-scaled ~9.9×, fell 83%).

`herdDistance` is therefore floored by `herdPackingFloor` — the radius at which the
herd is packed solid at the occupancy cap, times `config.social.herdPackingSlack`.
⚠ **Derived from `maxOccupantsPerCell` rather than being a constant of its own**,
because a constant fitted to today's herd sizes is precisely how the 2.0 came to be
wrong; the slack of 2 is validated against A83's measured table of how tightly real
bands stand and reproduces it to within ~10–20% from eight members up. 0 restores
the pre-fix arithmetic bit-for-bit and is the control arm. See
`social/herding.js`, and **A85** for the two instruments that now report a refused
step, neither of which existed while this was happening.
...
#### ✅ One pride, and it is an initial condition rather than an invariant — 2026-08-07 (PREDATOR-PLAN P1)

**Every lion in the world belongs to one pride**, and it took three species fields
and no mechanism: `cohort.groupSize` larger than any roster (one anchor, so one
record on tick 1), `inheritFromGuardian` (the config default, so cubs are born into
it), and **`groups.leavingSex: 'none'`** so nothing ever removes a living member.
`groups.maxMembers: 64` gives it room — measured, seed 42 reaches **exactly 8
members by tick 6000**, the config default, so without the raise the pride would
have started shedding cubs into second records right about there.

⚠ **It is deliberately temporary**, standing in until there are lion behaviours to
support multiple prides and male coalitions. What it gives up is stated: the
config's `leavingSex: 'male'` is what made a pride female-cored and matrilineal, and
restoring that is one field.

⚠⚠ **It is an initial condition plus no departures, not an enforced invariant.**
Nothing in `GroupSystem` knows this species wants one record. A lion that ends up
unattached — its record dissolved while it was the last survivor, or it was spawned
by command elsewhere — rejoins only by walking within `groups.joinRadius` (6) of a
pride-mate, and may found a second pride with another stray meanwhile. The test
asserts the founded world, not a property that holds under every history.
...
#### ✅ Cluster size solved from the roster, and clusters pushed apart — 2026-08-07 (PREDATOR-PLAN P2)

Two additions, both still placement and only placement.

**A species may derive its cluster size instead of stating one.** Declaring
`cohort.preferredGroupSize` and `cohort.maxGroups` makes the fixture solve
...
count**. At the hyena's 10 / 5 that is 20 founders ⇒ 2 clans of 10, 30 ⇒ 3 of 10,
60 ⇒ 5 of 12, 100 ⇒ 5 of 20. ⚠ A derived size **may exceed the species'
`groups.maxMembers`**, and the degradation is deliberate rather than clamped: the
registry enrols the cap and the surplus founds records of its own, so clan *count*
runs past `maxGroups` at very large rosters. Clamping the size instead would
silently break the "3–5 clans" half of the ask; this way the failure is more clans
rather than a crowd of unattached animals, and **nobody ends up unattached** in any
roster measured. It binds on no roster this world ships.

**Clusters of one species are pushed apart.** `config.cohorts.separated`
(world-level switch) and `cohort.separation` over `cohorts.minClusterSeparation`
(0, so no species is separated unless it asks) rejection-sample each anchor away
from that species' earlier ones. ⚠ **Same species only** — separating a clan from a
*herd* would be a statement about where predators start relative to prey, which is
a much larger claim. ⚠ The fallback is the **furthest candidate of `placementAttempts`**,
not the first: the constraint can be unsatisfiable and ending on the first draw
would throw the work away. ⚠ At separation 0, or for a species' first cluster, it
is a single `passableSpawnPosition` call and the `worldgen` stream is untouched —
which is what makes the off arm byte-identical rather than merely equivalent.

Measured on the hyena, ten seeds, nearest distance between two clan centres at
tick 1: **64 minimum / 105 mean with it, against 25 / 87 without**. It lifts the
worst case and barely moves the average, which is what a floor should do.

⚠⚠ **A sparser cluster is the failure mode here, not a denser one, and that is the
opposite of the intuition.** Both this phase and P1 raised a `spread` on the
reasoning that more animals need more ground, and both had to put it back: a
cluster founds **one** record only if it is connected within `groups.joinRadius`
transitively, so at spread 6 a ten-animal clan founded as `8+2` — a clan with a
stranded pair standing inside it, permanently, since records never merge. Measured
over ten seeds, hyena records on tick 1 from 20 founders: spread 2–3 gave one clan
on 10/10, spread 4 on 9, spread 5 on 5, spread 6 on 2. Shipped at 3, and the lion
at 4. Crowding is not the constraint — a radius-3 disc is ~28 cells at
...
#### ✅ v36 — the prey ceiling in force, and clan sizes _(2026-08-07, PREDATOR-PLAN P8)_

Two inspection-only additions, both answering a question the predator phases
created and neither in a bulk snapshot (a snapshot carries what a renderer draws;
an inspector answers questions about one animal).

**`predation` on entity inspection** — `solo`, `group`, and **`ceiling`**: the
ceiling *actually in force this tick*, plus the `backing` it was chosen by, the
`needed` threshold, and the `floor`. The cooperative ceiling (§1.1 A59) is
otherwise invisible: "why did that hyena walk past a zebra" and "because it was
alone" were two inferences and are now one look. ⚠ `null` for any species with an
empty `preySpeciesIds`, so the vulture reports nothing rather than a ceiling it has
no prey for. ⚠ `backing` is **last tick's** count, because perception reads the
social summary a phase before `SocialSystem` rewrites it — reporting a fresh count
beside a stale decision would be the more confusing of the two.

**`metrics.groups.meanSizeBySpecies`** — mean members per record, beside the
existing per-species *count*. The count says how many clans exist; this says how
big they are, and the second is what the cluster-size rule is judged by (§9
Founding cohorts): a roster that quietly drifted to pairs reads identically in
`bySpecies`. ⚠ A separate map rather than a richer `bySpecies`, because the
renderer's metrics panel reads that as a number.

⚠⚠ **Peak concurrent records is deliberately absent.** A maximum over time cannot
be recomputed from the world, so it would have to be accumulated and persisted —
and would read differently after a restore. Same reason `saturated` is a sample
rather than a count of refusals. A test asserts the absence so it reads as a
decision rather than a gap; sample the metric over a run if a peak is wanted.
...
coverPatchDensity: 0 }` — and each copy was complete on the day it was written and
silently incomplete the moment a terrain type arrived. Trees made that concrete: tree
counts are **absolute** like `ridges` and `thickets`, so the demo's 60 lone trees
landed unchanged in a 32×32 sandbox — **~12% of it, against 2.45% of the demo world**
...
| ⚠⚠ D42 | **An instrument has to name the animal and the mechanism, not the event — and four phases in a row named the event.** T3 asked for "carcasses lost to a stronger scavenger", which in this engine is the `entity.robbed` event — and that event needs a *holder*, so a leopard hauling a kill (not feeding, holding nothing) emits nothing and the theft that mattered most was invisible; measured that way, caching looked **worse**. F2 asked for "ticks from carcass creation to first feeder" and got a **flat** 193.9 against 192.3, because the first feeder at a body is usually whatever killed it — the number that moves is time to the first *vulture* (332.9 against 399.5). F1 asked for a flicker *rate* without saying what to break it down by, so 289-per-1000 read as an energetics problem when it was five misclassified actions. V1 asked for an occupancy share, which measured a cue rather than the roost the phase is named after | **Write the metric as a sentence containing the species and the mechanism, then check that no other species or mechanism can move it.** "Time to first feeder" is a fact about carcasses; "time to the first vulture at a carcass" is a fact about vultures, and only one of them is what flight is for. ⚠ The corollary that catches the T3 case: **an event is a record of a mechanism firing, not of the thing the mechanism is about** — check that the event you plan to count is actually emitted on the path you care about, before the run rather than after |
...
| ⚠⚠ D49 | **A number has geometric partners, and widening one of them is half a change.** `herdRadius` went 6 → 11 and decided *who* makes a herd's centre of mass; `herdDistance` decided how close an animal stands to that centre and stayed at the 2.0 chosen for the old radius. The pair describes a **density**, and the new one was ~8 bodies per cell against a cap of 2 — so the movement system refused 37.5% of wildebeest steps forever, they starved on top of forage, and two species lost 57% and 83% of their populations. Every individual test passed; the sweep gate passed | **When widening a radius, list the numbers that are compared against it and say what each one becomes.** ⚠ The fix is a floor **derived from the bound that was actually being violated** (`maxOccupantsPerCell`) rather than a new constant — a constant tuned against today's herd sizes is exactly how the 2.0 came to be wrong. ⚠⚠ The tell was available and unread: refusal rate is a *ratio*, so it needs no baseline to look wrong at 37.5% |
| ⚠⚠ D50 | **A fix can hide its own symptom from the instrument added to watch for it.** The crowd-lock branch resolves a jam by reporting `moving: false` — so those ticks leave both the numerator and the denominator of the refusal rate, and **a world jamming harder reports a falling refusal rate**. The metric and the detector would both have gone quiet at exactly the moment they mattered | **After adding a mitigation, re-ask whether the measurement still counts the thing the mitigation handles.** A mitigated occurrence is still an occurrence and must reach the counter — here via an explicit `crowdLocked` flag, counted into both terms. ⚠ Same family as D40's null arm and A83's blind detector: the failure is always a clean report that means nothing |
| ⚠ D51 | **Extending a tool to the risks a plan named is necessary and not sufficient.** A83 added four detectors for the four risks the behaviour plan listed, and all four stayed clean while an unlisted one cost two species. The evidence was in the tool's own output the whole time — `starved with 122 biomass within 2c`, and one animal with `294 refused steps` — in a field nothing was thresholded on | **Ask which field the tool already prints that nothing triggers on.** A number worth printing in a detail string is a number somebody thought was diagnostic; if no detector reads it, that is a gap with a ready-made threshold. ⚠ Promoting one is far cheaper than inventing a detector, and it is calibrated by the reports already in hand |
| ⚠⚠ D52 | **A continuous-sounding cue can be read once per commitment.** The migration drift is documented and tuned as "0.5 × thirst, harder the thirstier", and it was blended into a heading on exactly one tick out of every 8–24 — the tick a wander commitment was freshly taken. Every unit test of the *cue* passed, because the cue was correct; the animals still died of thirst crossing a map with a lake on it | **For any influence, ask how often the thing it influences is actually recomputed.** A weight and a read-rate multiply, and only the weight is visible in the config. ⚠ Same family as A65 — the recovery written where nobody reads it — and the tell is identical: a mechanism that measures healthy in isolation and absent in the world |
| ⚠⚠ D53 | **Fixing a search cue can raise the death toll it was meant to lower, and the sign depends on the map.** Making the water drift actually reach animals cut dehydration on worlds with big lakes (11 → 6, 7 → 2) and *raised* it on the world whose lake is 65 cells (136 → 150 → 171 as the cue strengthened), because a shore `maxOccupantsPerCell` cannot admit becomes a queue. Monotone in both directions at once | **A cue that concentrates animals has a capacity term, and one seed cannot show it.** Sweep any "help them find X" change across maps where X is scarce *and* abundant before reading a mean. ⚠ It also bounds the lever: past a point, more cue stops meaning "find the water" and starts meaning "queue for it", which is the argument for a directed action rather than a stronger bias |
| ⚠ D54 | **A detector's exclusion list is where its calibration actually lives.** `circling-in-need` had a search-fraction term added specifically to stop it reading "working a home range" as "stuck" — and `wander`, the action meaning *no drive at all*, was in the set of "searching" actions. Predators wander 56–95% of their ticks, so the guard was satisfied by their resting state and the detector flagged 53–100% of them in every healthy world | **When a detector over-fires on one group, check what its terms mean *for that group* before making a threshold species-relative.** ⚠ Here the principled-looking fix was measured and was wrong: need ≥ 0.4 is reached by vulture 33% / lion 39% against buffalo 43% — the guilds barely differ, and a per-species need bound would have discriminated nothing while looking rigorous |
| ⚠⚠ D55 | **A cluster that is too *sparse* fails to become a group; crowding is not the constraint.** Twice in one plan a founding `spread` was raised on the reasoning that more animals need more ground, and both times it had to be put back. A cohort founds **one** record only if it is connected within `groups.joinRadius` *transitively*, so a ten-animal clan at spread 6 founded as `8+2` — a clan with a stranded pair inside it, permanently, because records never merge. Measured, one clan on 10/10 seeds at spread 2–3 and 2/10 at spread 6 | **A radius and the density it implies are one decision, not two** (this is A84's shape at founding rather than at runtime). Before widening any placement radius, ask what *joins* the animals at the far edge to the ones at the near edge — and check the arithmetic: a radius-3 disc is ~28 cells at 2 occupants each, so ten animals use a sixth of it. The failure is never that they do not fit |
| ⚠⚠ D56 | **Two numbers chosen from first principles would have shipped as dead code, and only measurement caught either.** A cooperative-hunting threshold was set at 4 band-mates; hyenas have ≥4 in **1% of adult ticks**, so it would have fired essentially never. A lion group prey-ceiling was drafted *above* its own solo ceiling, which no prey in the world lies between. Both are D43's lesson in a new plan | **Before shipping a threshold, measure the distribution it sits in; before shipping a second bound, check that something lies between it and the first.** A weight whose off-arm cannot be distinguished from its on-arm is not conservative, it is absent |
...
### The plans, and what each retired

Five numbered plans have run to completion. Each is retired to `legacy-docs/` and
what it built is described in this document; read them for *why* a decision was
made and for the **As built** notes where a phase's own prediction turned out
wrong.

| plan | retired | built | left open |
| --- | --- | --- | --- |
| `PLAN.md` | — | the engine, 30 steps | §1.5 |
| `PLAN-SPECIES.md` | 2026-07-31 | three species → **eight** | §1.3 |
| `TREES-FLIGHT-VULTURE-PLAN.md` | 2026-08-04 | trees, elevation, kill caching, flight | **A77**, **A78** |
| `BEHAVIOR-PLAN.md` | 2026-08-06 | herbivore sociality that steers movement | **A43**, **A82** |
| **`PREDATOR-PLAN.md`** | **2026-08-07** | **lion and hyena sociality: one pride, clans solved from the roster, cooperative prey ceilings, coordinated stalking, pride territory, numbers at a carcass** | **A91**–**A94** |

**PREDATOR-PLAN** (eight phases, all shipped) closed two items that had been open
since 2026-07-30 — **A59** (a pride can now take prey a lone lion refuses) and
**A60** (a group can now hold ground) — and **closed both by *not* building the fix
the item specified**, which is the part worth carrying forward. A59 priced its fix
at "teaching the perception hot loop about company"; that cost had already vanished
when an earlier phase started publishing a band-mate count for every animal. A60
called for keying the claim layer on `groupRecordId`; that was built and discarded,
because a stored group id is a second copy of membership that outlives the record
it names. ⚠ **Re-read what an old item assumes about cost before quoting it.**

Its lessons are **D55–D59**; two of its asks were found structurally inexpressible
and are recorded in §1.3 with the nearest legal thing to each.
...
`habitat`, and — from 2026-08-06 — `consensus` and `charge`). ⚠ **`charge` is the
sharpest case of the *machinery* half so far**: its two numbers are not tuning at all
but **bounds** — how much stamina a defender must keep back, and the longest a pursuit
may run — and they are world-level precisely because a species that declares
`behavior.chargeWeight` is declaring an action that outranks fleeing. A bound a
species file could raise is not a bound. The biology sits beside them in an always-per-species **field**
...
#### ⚠⚠ What `--set` / `--controlSet` cannot reach _(2026-08-07, D58)_

The sweep's control arms set **`config`** keys. A species block **beats** the config
(§8), so anything stated inside one — `preySpeciesIds`, the `predation` ratios,
`territory.defends`, `groups.*`, `behavior.*` weights — **has no off switch and
cannot be A/B'd from the command line.** This is not a gap in the sweep; it is the
§8 precedence rule seen from the measurement side.

Two consequences, both learned the hard way:

- **Put a mechanism's switch in a global config section**, always, so its arm is
  reachable. Every mechanism that did this in PREDATOR-PLAN was measured with one
  command; every one that did not needed the tree edited.
- **Where biology genuinely must live in a species block, measure the control arm
  *before* applying the change.** Population counts are deterministic in seed and
  code, so there is no drift to correct for and nothing to restore. ⚠ Swapping
  species files under running jobs is what lost an edit silently and sent two tests
  failing against code that was never on disk.
...
⚠⚠ **The same budget applies per net-new *mechanism*, and the six phases of
2026-08-03/04 (trees, elevation, kill caching, flight, the flying vulture, its
woodland preference) are the evidence.** Each cost something the plan behind them
did not predict: T1 collapsed a habitat assertion (**A72**), T3's stated instrument
could not see its own claim, F2's gate had to be **re-run** because a behavioural
correction landed while it was going, and V1's headline measurement failed a null
control (**D40**). ⚠ Two additions to the order above, both learned there:

- **Take every cheap behavioural measurement before starting the gate, not beside
  it.** A behaviour change invalidates a gate in progress, and the twenty minutes
  are spent either way.
- ⚠⚠ **A phase whose entire content is a species-file edit has no reproducible
  control**, because `--set`/`--controlSet` reach config sections and a species
  block beats the config (§8). The control is then a **copied tree** with the
  species file reverted — which doubles the gate, and which `git stash` cannot
  produce while more than one phase is uncommitted, since stash reverts to HEAD
  rather than to the previous phase. Budget the copy.
