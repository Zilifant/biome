...
| ------ | --------------------------------------------------------------------------------------------------------------------- | ---- | -------------- |
| **0**  | Guard rails: fix the comment-stripping in all three source scans (§5.5); rewrite the two "every species differs" assertions (§5.6); **re-baseline `npm run benchmark`** | none | test only |
| **1**  | **Bug-fix + mass audit.** §5.1 mass-scale `intakeRate`; §5.2 carcass nutrient return; §5.3 per-species `foodMinLevel`; §5.8 load-time speciesId check; the §4 constant audit with a written verdict each. Add `feeding` + `hunting` to `SPECIES_BLOCKS` | low | schema + real fixes |
| **2**  | Split `config.decision` → `config.behavior` (per-species) + `config.decision` (global); add `behavior` to `SPECIES_BLOCKS`; dedupe `drinkRange` (§3.1, §5.4) | med | schema + ~2 test fixes |
