 ...
 * simulation).
 *
 * Ownership: writes only `world.vegetation` biomass (regrowth). Consumption is
 * a separate concern (feeding, Step 9). No randomness — regrowth is
...
export class VegetationSystem extends SimulationSystem {
   ...
   */
  constructor({ growthRate = 0.08, seedFloor = 0.08, updateInterval = 5 } = {}) {
  ...
  update(world) {
    world.vegetation.grow({ growthRate: this.growthRate, seedFloor: this.seedFloor });
  }
}
